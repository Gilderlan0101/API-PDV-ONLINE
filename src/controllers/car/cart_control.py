...
class CartManagerDB:
    ...
    async def _get_caixa_ativo(self) -> Caixa:
        ...
        try:
            ...
            # Busca no banco
            caixa = await Caixa.filter(usuario_id=self.company_id, funcionario_id=self.employee_id, aberto=True).select_related('funcionario').first()

            if not caixa:
                logger.warning(f"Caixa não encontrado - Empresa: {self.company_id}, Funcionário: {self.employee_id}")
            ...
            logger.info(f"Caixa {caixa.caixa_id} carregado para empresa {self.company_id}")
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao acessar caixa.")
    ...
    async def _get_produto(self, product_id: int) -> Produto:
        ...
        produto = await Produto.filter(id=product_id, active=True, usuario_id=self.company_id).first()

        if not produto:
            logger.warning(f"Produto {product_id} não encontrado para empresa {self.company_id}")
    ...
    def _calcular_total(self, price: Decimal, quantity: int, discount: Decimal = Decimal('0'), addition: Decimal = Decimal('0')) -> Decimal:
        """
    ...
    async def add_produto(self, product_id: int, quantity: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if produto.stock < quantity:
                logger.warning(f"Estoque insuficiente - Produto: {product_id}, Requerido: {quantity}, Disponível: {produto.stock}")
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Estoque insuficiente. Disponível: {produto.stock}")
            ...
            if cart_item:
                ...
                logger.info(f"Atualizando item existente - Produto: {product_id}, Quantidade adicional: {quantity}")
                return await self.update_produto(product_id=product_id, quantity=quantity, replace_quantity=False)  # Adiciona à quantidade existente
            ...
            await produto.save()
            self._produto_cache.pop(f"prod_{self.company_id}_{product_id}", None)  # Invalida cache

            logger.info(f"Produto adicionado - ID: {cart_item.id}, Produto: {product_id}, Quantidade: {quantity}")
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao adicionar produto ao carrinho.")
    ...
    async def update_produto(
        ...
        try:
            ...
            if not cart_item:
                logger.warning(f"Item não encontrado no carrinho - Produto: {product_id}")
            ...
            if quantity is not None:
                new_quantity = await self._processar_quantidade(cart_item, produto, quantity, replace_quantity, old_quantity)
                cart_item.quantity = new_quantity

            # Processa desconto
            if discount is not None:
                new_discount = await self._processar_valor_monetario(cart_item.discount, discount, replace_discount, "discount")
                cart_item.discount = float(new_discount)

            # Processa acréscimo
            if addition is not None:
                new_addition = await self._processar_valor_monetario(cart_item.addition, addition, replace_addition, "addition")
            ...
            cart_item.total_price = float(self._calcular_total(price_decimal, cart_item.quantity, discount_decimal, addition_decimal))
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao atualizar produto no carrinho.")

    async def _processar_quantidade(self, cart_item: CartItem, produto: Produto, quantity: int, replace: bool, old_quantity: int) -> int:
        """
        ...
        if new_quantity <= 0:
            logger.info(f"Removendo produto por quantidade zero - Produto: {produto.id}")
            if produto:
                produto.stock += old_quantity
                await produto.save()
                self._produto_cache.pop(f"prod_{self.company_id}_{produto.id}", None)
            await cart_item.delete()
            raise HTTPException(status_code=status.HTTP_200_OK, detail="Produto removido do carrinho por quantidade zero.")
        ...
        if quantity_difference > 0 and produto.stock < quantity_difference:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Estoque insuficiente para aumentar quantidade. Disponível: {produto.stock}"
            )
    ...
    async def _processar_valor_monetario(self, current_value: float, new_value: float, replace: bool, field_name: str) -> Decimal:
        """
        ...
        if new_decimal < 0:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"{field_name.capitalize()} não pode ser negativo.")
        ...
        try:
            ...
            for item in produtos:
                ...
                if key in agrupados:
                    ...
                    agrupados[key]['discount'] = max(agrupados[key]['discount'], float(item.discount or 0))
                    agrupados[key]['addition'] = max(agrupados[key]['addition'], float(item.addition or 0))
                else:
                    agrupados[key] = {
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao listar produtos do carrinho.")
    ...
    async def remove_produto(self, product_id: int) -> Dict[str, Any]:
        ...
        try:
            caixa = await self._get_caixa_ativo()
            cart_items = await CartItem.filter(caixa_id=caixa.caixa_id, product_id=product_id).all()
            ...
            if produto:
                ...
                await produto.save()
                self._produto_cache.pop(f"prod_{self.company_id}_{product_id}", None)

            # Remove itens
            deleted_count = await CartItem.filter(caixa_id=caixa.caixa_id, product_id=product_id).delete()

            logger.info(f"Produto removido - ID: {product_id}, Quantidade restaurada: {quantidade_total}")
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao remover produto do carrinho.")
    ...
    async def limpar_carrinho(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Restaura estoques
            for item in itens:
                produto = await Produto.get_or_none(id=item.product_id, usuario_id=self.company_id)
                if produto:
                    produto.stock += int(item.quantity or 0)
                    await produto.save()
                    self._produto_cache.pop(f"prod_{self.company_id}_{item.product_id}", None)
            ...
            return {"success": True, "aviso": "Carrinho limpo e estoque restaurado.", "itens_removidos": len(itens)}

        except Exception as e:
            logger.error(f"Erro ao limpar carrinho: {str(e)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao limpar carrinho.")
    ...
    async def limpar_carrinho_pos_venda(self, caixa_id: int) -> bool:
        ...
        try:
            ...
            if itens:
                ...
                logger.info(f"Carrinho pós-venda limpo - Caixa: {caixa_id}, Itens: {len(itens)}")
                return True
            return False
        except Exception as e:
            logger.error(f"Erro ao limpar carrinho pós-venda {caixa_id}: {str(e)}")
            return False
        ...
        try:
            ...
            return {
                ...
                "resumo": {
                    ...
                    "desconto_total": float(total_desconto.quantize(Decimal('0.01'))),
                    "acrescimo_total": float(total_acrescimo.quantize(Decimal('0.01'))),
                    "total_geral": float(total_geral.quantize(Decimal('0.01'))),
                    "quantidade_itens": total_itens,
                    "quantidade_produtos": len(itens),
                    "subtotal_formatado": self._formatar_moeda(float(subtotal)),
                    "desconto_formatado": self._formatar_moeda(float(total_desconto)),
                    "acrescimo_formatado": self._formatar_moeda(float(total_acrescimo)),
                    "total_formatado": self._formatar_moeda(float(total_geral)),
                },
            }

        except Exception as e:
            logger.error(f"Erro ao gerar resumo do carrinho: {str(e)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao gerar resumo do carrinho.")
