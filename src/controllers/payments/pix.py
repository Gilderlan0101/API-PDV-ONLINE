...
class PixCreateRequest(BaseModel):
    ...
    def validate_full_name(cls, v):
        ...
        if len(v.replace(' ', '')) < MIN_NAME_SIZE:
            raise ValueError(f'Nome deve ter pelo menos {MIN_NAME_SIZE} caracteres (sem espaços)')
        return v.title()
    ...
    def validate_key_pix(cls, v):
        ...
        if re.match(phone_pattern, v.replace(' ', '').replace('-', '').replace('.', '')):
            return v
        ...
        raise ValueError('Chave PIX inválida. Use CPF, CNPJ, email, telefone ou chave aleatória')
...
class PixService:
    ...
    async def generate_qr_code(self, pix_data: GenerateQRCodeFor) -> PixQRCodeResponse:
        ...
        try:
            ...
            if not selected_key:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Nenhuma conta PIX selecionada. Selecione uma conta primeiro.")

            # Obtém os dados da conta selecionada
            selected_account = await Pix.filter(usuario_id=self.user_id, key_pix=selected_key, is_active=True).first()

            if not selected_account:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conta PIX selecionada não encontrada")
            ...
            logger.info(f"Gerando QR Code PIX para usuário {self.user_id}, valor: R$ {pix_data.value:.2f}")
            ...
            if pix_data.type_exit == 'qr':
                ...
                return PixQRCodeResponse(
                    ...
                    transaction_data={"value": pix_data.value, "receiver": selected_account.full_name, "key_pix": selected_key},
                )
    ...
    async def deactivate_pix_account(self, pix_id: int) -> bool:
        ...
        try:
            ...
            if not pix_account.is_active:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Conta PIX já está desativada")
    ...
    async def select_account_default(self, select_an_account_id: int) -> str:
        ...
        try:
            ...
            if not selected_account:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conta PIX não encontrada ou não está ativa")
            ...
            logger.info(f"Conta PIX {select_an_account_id} selecionada como padrão para usuário {self.user_id}")
