...
class ViewsAllDepts:
    ...
    async def get_customer_debt(self):
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro ao processar dívida: múltiplos registros encontrados."
            )
    ...
    async def get_current_debts(self) -> dict:
        ...
        try:
            ...
            debts = await Partial.filter(usuario_id=self.company_id, value__gt=0).all()  # Apenas dívidas com valor maior que 0
            ...
            LOGGER.info(f"Encontradas {len(result)} dívidas ATIVAS para usuário {self.company_id}")
    ...
    async def get_paid_debts(self) -> dict:
        ...
        try:
            paid_debts = await finished_debts.filter(usuario_id=self.company_id).all()
            result = []
            ...
            LOGGER.info(f"Encontradas {len(result)} dívidas pagas para usuário {self.company_id}")

            return {'status': 200, 'dividas_pagas': result, 'total': len(result)}

        except Exception as error:
            LOGGER.error(f"Erro ao buscar dívidas pagas: {str(error)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao buscar dívidas pagas")
