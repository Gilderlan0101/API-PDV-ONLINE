...
class PartialPayment:
    ...
    def _validate_input_data(self) -> None:
        ...
        if self.value_received is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Valor recebido é obrigatório")

        try:
            paid_value = Decimal(str(self.value_received))
            if paid_value <= 0:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Valor recebido deve ser maior que zero")
        except (InvalidOperation, ValueError):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Valor recebido inválido")

    async def _get_debt_record(self) -> Partial:
        """Busca o registro específico da dívida pelo CPF"""
        try:
            debt_record = await Partial.filter(usuario_id=self.user_id, cpf=self.cpf).first()

            if not debt_record:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Nenhuma dívida encontrada para este CPF")

            return debt_record

        except Exception as error:
            LOGGER.error(f"Erro ao buscar dívida específica: {str(error)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao buscar dívida")
    ...
    async def _register_payment_history(self, debt_record: Partial, current_balance: Decimal, remaining_balance: float):
        ...
        try:
            ...
            if hasattr(debt_record, 'payment_history') and debt_record.payment_history:
            ...
            await Partial.filter(id=debt_record.id).update(payment_history=history)

        except Exception as error:
            LOGGER.error(f"Erro ao registrar histórico: {str(error)}")
            # Não interrompe o fluxo principal se houver erro no histórico

    async def _update_or_remove_debt(self, debt_record: Partial, remaining_balance: float, current_balance: Decimal) -> dict:
        try:
            if remaining_balance <= 0:
                ...
                return {
                    ...
                    "troco": abs(remaining_balance) if remaining_balance < 0 else 0,
                ...
                await Partial.filter(id=debt_record.id).update(
                    value=new_value, payment_method=self.payment_method  # Garantir que seja 0 se remaining_balance for <= 0
                )
            ...
            return {
                ...
                "valor_pago": float(self.value_received),
                "valor_original": float(current_balance),
                "cliente": updated_record.customers_name if updated_record else debt_record.customers_name,
            }

        except Exception as error:
            LOGGER.error(f"Erro ao atualizar/remover dívida: {str(error)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro ao processar pagamento")

    async def _move_to_paid_debts(self, debt_record: Partial, original_balance: Decimal):
    ...
    async def get_current_debts(self) -> dict:
        ...
        try:
            ...
            LOGGER.info(f"Encontradas {len(result)} dívidas para usuário {self.user_id}")

            return {'status': 200, 'dividas': result, 'total': len(result)}

        except Exception as error:
            LOGGER.error(f"Erro ao buscar dívidas [ PROCESS_PARTIAL_PAYMENTS (get_current_debts) ] : {str(error)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao buscar dívidas")
