    ...
    try:
        ...
        for item in itens:
            try:
                ...
                preco_unitario = preco_total / quantidade if quantidade > 0 else 0
                ...
                venda_itens.append(
                    {
                        "product_name": item.get("product_name", "Produto não informado"),
        ...
        receipt_data = {
            "Nota Fiscal": {
                "Empresa": {
                    "Razão Social": get_usuario_attr("company_name", "Não informado"),
                    "Nome Fantasia": get_usuario_attr("trade_name", "Não informado"),
                    "CNPJ": get_usuario_attr("cnpj", "Não informado"),
                    "Endereço": endereco,
                    "Inscrição Estadual": get_usuario_attr("state_registration", "Não informado"),
                    "Inscrição Municipal": get_usuario_attr("municipal_registration", "Não informado"),
                    "Operado por": funcionario_nome or get_usuario_attr("username", "Não informado"),
        ...
        if payment_upper == 'DINHEIRO':
            if valor_recebido is not None:
                pagamento_info["Valor Recebido"] = f'R$ {float(valor_recebido):.2f}'
            else:
        ...
        if valor_recebido is not None:
            receipt_data["Nota Fiscal"]["valor_recebido"] = float(valor_recebido)
        if troco is not None:
