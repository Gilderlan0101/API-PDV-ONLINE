...
async def processar_venda_carrinho(
    ...
    sale_code: Optional[str] = None,  # 🔹 NOVO: Receber sale_code como parâmetro
) -> dict:
    """
    ...
    print(f"DEBUG: user_id={user_id}, cart_items.len={len(cart_items)}, method={payment_method}")
    ...
    try:
        ...
        for i, item in enumerate(cart_items):
            ...
            quantity = int(item.get("quantity", 1)) if is_dict else getattr(item, "quantity", 1)
            ...
            if not product_code:
                print(f"DEBUG Item {i+1}: Pulando. product_code é None ou vazio.")
                continue
            ...
            if not produto:
                print(f"DEBUG Item {i+1}: Pulando. Produto NÃO ENCONTRADO no DB.")
                continue
    ...
    checkout_instance.sale_code = sale_code  # 🔹 CORREÇÃO: Definir sale_code no checkout
    ...
    print(f"DEBUG: Venda verificada no DB: Sale_Code={venda_verificada.sale_code}")
