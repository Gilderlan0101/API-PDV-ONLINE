...
class StockExit:
    ...
    def check_fields(self) -> None:
        ...
        if not self.company_id or self.company_id <= 0:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="ID da empresa inválido.")
        if not self.product_code and not self.product_name:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="É obrigatório fornecer o código OU o nome do produto.")
    ...
    async def remove_product_total(self) -> dict:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao registrar produto arquivado: {str(e)}")
