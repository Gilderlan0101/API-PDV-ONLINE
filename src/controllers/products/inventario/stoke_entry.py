...
class EntryProducts:
    ...
    def check_fields(self) -> None:
        ...
        if not self.company_id or self.company_id <= 0:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="ID da empresa (company_id) é inválido ou ausente.")
        ...
        if self.new_stock <= 0:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A nova quantidade de estoque (new_stock) deve ser maior que zero.")
    ...
    async def _find_product_id(self) -> Optional[int]:
        ...
        target_company_name = company_owner.company_name if company_owner and company_owner.company_name else None
        ...
        try:
            ...
            if self.product_code:
                ...
                search_result = await get_product_by_user(user_id=self.company_id, code=self.product_code)
            ...
            # 3. Busca por Nome (Usa deep_search, que retorna tipicamente List[Dict])
                ...
                results_list = await deep_search(user_id=self.company_id, product_name=self.product_name, target_company=target_company_name)

                # CORREÇÃO: Se for uma lista de resultados e o primeiro item for um dict, extrai o 'id'
                if results_list and isinstance(results_list, list) and isinstance(results_list[0], dict) and results_list[0].get('id'):
                    return results_list[0].get('id')

            return None

        except Exception as e:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro interno na busca do produto: {e}")
    ...
    async def update_stock(self) -> dict:
        ...
        if not product_id:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Erro: Produto não encontrado ou o ID não pôde ser determinado.")
        ...
        try:
            ...
            rows_updated = await Produto.filter(id=product_id, usuario_id=self.company_id).update(**update_fields)

            # 4. Verifica o resultado
            if rows_updated > 0:
                return {"message": "Atualização de estoque realizada com sucesso.", "product_id": product_id, "status_code": status.HTTP_200_OK}
            else:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro interno ao finalizar a atualização do estoque: {e}")
