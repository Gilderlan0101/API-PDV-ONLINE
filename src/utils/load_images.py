...
async def load_imgs(path: str, store_id: int, product_id: Optional[int] = None) -> dict:
    """
    ...
    try:
        ...
        if product.image_url:
            return {"success": True, "message": "Produto já possui imagem", "existing_image": product.image_url}
        ...
        if file_ext not in allowed_extensions:
            return {"success": False, "error": f"Formato {file_ext} não suportado. Use: {', '.join(allowed_extensions)}"}
        ...
        return {"success": True, "message": f"Imagem padrão adicionada com sucesso", "product_id": product_id, "image_path": path}
