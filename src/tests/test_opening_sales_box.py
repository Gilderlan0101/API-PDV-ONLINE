...
class TestCashControllerDebug(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_simple_case(self, mock_employee_get):
        ...
        # ACT & ASSERT
        try:
            result = await CashController.abrir_caixa(funcionario_id=2, saldo_inicial=12.50, nome='Maria', company_id=1)
    ...
    async def test_with_context_manager(self):
        ...
        with patch('src.controllers.caixa.cash_controller.Employees.get_or_none') as mock_employee_get:
            ...
            with patch('src.controllers.caixa.cash_controller.Caixa.filter') as mock_caixa_filter:
                ...
                with patch('src.controllers.caixa.cash_controller.Caixa.create') as mock_caixa_create:
                    mock_caixa = AsyncMock()
                    mock_caixa_create.return_value = mock_caixa

                    with patch('src.controllers.caixa.cash_controller.CashMovement.create'):
                        # ACT
                        result = await CashController.abrir_caixa(funcionario_id=2, saldo_inicial=12.50, nome='Maria', company_id=1)
