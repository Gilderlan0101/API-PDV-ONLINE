...
async def collection_of_the_day(current_user: SystemUser = Depends(get_current_user)):
    '''
...
async def closing_checkout(
    ...
    try:
        ...
        request = await CashController.close_checkout(employe_id=employe_id, checkout_id=checkout_id, company_id=current_user.empresa_id)
        ...
        cash = await Caixa.filter(usuario_id=user.id, caixa_id=checkout_id).first()

        if not cash:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Caixa com ID {checkout_id} não encontrado")
        ...
        infos = {
            ...
            "data": {'Nome': cash.nome, 'ID': cash.id, 'Caixa_id': cash.caixa_id, 'Aberto': cash.aberto, 'Saldo_atual': saldo_formatado},
        }
        ...
        LOGGER.error(f'Erro na rota [CHECKOUT/FECHAMENTO]: {e} :: {e.__class__.__name__}')
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno do servidor ao processar o fechamento do caixa")
