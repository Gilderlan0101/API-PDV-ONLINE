...
async def registre_sale_in_partial(data: InputData, current_user: SystemEmployees = Depends(get_current_employee)):
    """
...
async def paids(current_user: SystemUser = Depends(get_current_user)):
    ...
    '''
    var = ViewsAllDepts(company_id=current_user.empresa_id, user_id=current_user.empresa_id)

    return await var.get_paid_debts()


@partial.put('/atualiza-valor')
async def update_pending_debt(data: ReceivePaymentPartial, SystemEmployees: SystemUser = Depends(get_current_employee)):
    '''Comportamento:
    ...
    try:
        ...
        if data.type_meyhod_payment.upper() in __PAYMENT_METHODS:
            ...
            update_value = PartialPayment(
                value_received=data.value_received, cpf=data.cpf, user_id=SystemEmployees.empresa_id, payment_method=data.type_meyhod_payment
            )
