...
from src.controllers.payments.pix import PixService, PixCreateRequest, PixQRCodeResponse, GenerateQRCodeFor
from src.schemas.payments.accounts_pix import PixAccountResponse, PixAccountsList
...
async def create_pix_account(pix_data: PixCreateRequest, current_user: SystemUser = Depends(get_current_user)):
    """
...
async def generate_pix_qrcode(pix_data: GenerateQRCodeFor, current_user: SystemUser = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao gerar QR Code")
...
async def account_default(
    select: int = Query(..., description="ID da conta PIX a ser selecionada como padrão"), current_user: SystemUser = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return {'sucesso': True, 'message': 'Conta PIX selecionada com sucesso', 'chave_pix_selecionada': selected_key}
...
async def delete_pix(account: int = Query(..., description="ID da conta PIX a ser excluída"), current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if remove_account:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Falha ao excluir conta PIX")

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro interno ao excluir conta PIX")
