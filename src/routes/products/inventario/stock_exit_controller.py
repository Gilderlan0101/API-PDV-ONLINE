...
async def stock_exit_products(
    action: str = Query(..., description="Ação a ser executada: 'partial' (baixa de quantidade) ou 'total' (remoção/arquivamento total)."),
    product_code: Optional[str] = Query(None, description="Código do produto."),
    product_name: Optional[str] = Query(None, description="Nome do produto (alternativa ao código)."),
    quantity_to_remove: int = Query(0, description="Quantidade a ser removida (obrigatório para action='partial')."),
    detail: Annotated[Optional[str], Query(description='Descrição/motivo da saída ou arquivamento.')] = None,
    ...
    try:
        ...
        stock_exit = StockExit(
            company_id=company_id, product_code=product_code, product_name=product_name, quantity_to_remove=quantity_to_remove, detail=detail
        )
        ...
        if action.lower() == 'partial':
            ...
            if quantity_to_remove <= 0:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A ação 'partial' requer 'quantity_to_remove' maior que zero.")
            ...
            if not detail:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A ação 'total' requer um 'detail' (motivo) para arquivamento.")

            return await stock_exit.remove_product_total()

        else:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Ação inválida. Use 'partial' ou 'total'.")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro interno ao processar a saída de estoque: {str(e)}")
