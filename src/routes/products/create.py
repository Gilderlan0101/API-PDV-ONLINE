...
async def create_product(
    ...
    try:
        ...
        product_code = prod.product_code if prod.product_code else gerar_codigo_venda()
        barcode = prod.lot_bar_code if prod.lot_bar_code else lot_bar_code_size()
        ...
        if not image_url:
            ...
            try:
                result = await load_imgs(path=PATH_IMG, store_id=current_user.empresa_id, product_id=register_prod.id)
