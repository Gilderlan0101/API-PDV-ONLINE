...
async def list_all_products(current_user: SystemUser = Depends(get_current_user)):
    ...
    try:
        ...
        return {"success": True, "data": products_data, "error": None, "source": "database"}  # Para debug
...
@i_request
async def list_products_for_employee(current_employee: SystemEmployees = Depends(get_current_employee)):
    """
    ...
    try:
        ...
        employee = await Employees.filter(id=current_employee.id).select_related('usuario').first()
