...
async def delete_ticket(ticket_id: int, current_user: Usuario = Depends(get_current_user)):
    ticket = await Ticket.filter(id=ticket_id, usuario_id=current_user.id).first()
    if not ticket:
