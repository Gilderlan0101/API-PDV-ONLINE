...
async def cancel_sale(
    ...
    try:
        ...
        sale = await Sales.filter(sale_code=body.code.strip(), usuario_id=current_user.id).first()  # 🔹 CORREÇÃO: campo sale_code existe
        ...
        if not sale:
            ...
            for s in all_sales:
                print(f"  - Venda ID: {s.id}, Sale_code: {s.sale_code}, Produto: {s.product_name}")

            return {"success": False, "data": None, "error": f"Venda não encontrada. Código: {body.code}"}

        print(f"✅ Venda encontrada: ID {sale.id}, Produto: {sale.product_name}, Sale_code: {sale.sale_code}")
        ...
        if not product:
            ...
            product = await Produto.filter(usuario_id=current_user.id, name=sale.product_name).first()

        if not product:
            return {"success": False, "data": None, "error": f"Produto não encontrado para a venda {body.code}"}

        print(f"✅ Produto encontrado: {product.name}, Estoque atual: {product.stock}")
        ...
        print(f"📦 Estoque restaurado: +{quantidade_restaurada} unidades. Novo estoque: {product.stock}")
        ...
        if hasattr(Sales, 'cancelada'):
            ...
            sale.data_cancelamento = datetime.now(ZoneInfo('America/Sao_Paulo'))
            await sale.save()
