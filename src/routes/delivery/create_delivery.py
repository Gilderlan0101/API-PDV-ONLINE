from src.controllers.delivery.delivery_reports import gerenciagelivery, assign_delivery_to_driver, update_delivery_status, get_driver_deliveries
...
async def create_delivery(data: DeliveryCreate, current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        await delivery_controller.check_if_cliente_is_registered(data.customer_id, current_user.id)

        # Adicionar entregador se especificado
        if data.delivery_man_id:
            await delivery_controller.add_delivery_man(current_user.id, data.delivery_man_id)

        # Criar registro da entrega
        result = await delivery_controller.create_delivery_record(current_user.id, current_user.id)

        return {'status': 200, 'mensagem': 'Entrega criada com sucesso!', 'delivery_data': result}
...
async def assign_delivery_to_driver_route(delivery_id: int, driver_id: int, current_user: SystemUser = Depends(get_current_user)):
    """
    Endpoint para atribuir uma entrega específica a um entregador.
    """
    try:
        result = await assign_delivery_to_driver(delivery_id=delivery_id, driver_id=driver_id, company_id=current_user.id)
        return result
...
async def assign_specific_delivery_route(delivery_id: int, driver_id: int, current_user: SystemUser = Depends(get_current_user)):
    """
    Endpoint para atribuição específica de uma entrega a um entregador.
    """
    try:
        result = await assign_specific_delivery(delivery_id=delivery_id, driver_id=driver_id, company_id=current_user.id)
        return result
...
async def update_delivery_status_route(delivery_id: int, new_status: str, current_user: SystemUser = Depends(get_current_user)):
    """
    Endpoint para atualizar o status de uma entrega.
    """
    try:
        result = await update_delivery_status(delivery_id=delivery_id, new_status=new_status, company_id=current_user.id)
        return result
...
async def get_driver_deliveries_route(driver_name: str, current_user: SystemUser = Depends(get_current_user)):
    """
    Endpoint para obter todas as entregas de um entregador específico.
    """
    try:
        result = await get_driver_deliveries(driver_name=driver_name, company_id=current_user.id)
        return result
...
async def get_delivery_status_report_route(current_user: SystemUser = Depends(get_current_user)):
    """
...
async def update_payment_status_route(sales_id: int, new_status: str, current_user: SystemUser = Depends(get_current_user)):
    """
    Endpoint para atualizar o status de pagamento de uma venda.
    """
    try:
        result = await update_payments_status(company_id=current_user.id, sales_id=sales_id, new_status=new_status)
        return result
...
async def get_total_sales_route(current_user: SystemUser = Depends(get_current_user)):
    """
...
async def get_sales_quantity_route(current_user: SystemUser = Depends(get_current_user)):
    """
...
async def get_active_deliveries_route(current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if management_data['success']:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=management_data.get('error', 'Erro ao buscar entregas ativas')
            )
...
async def get_pending_deliveries_route(current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if management_data['success']:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=management_data.get('error', 'Erro ao buscar entregas pendentes')
            )
...
async def get_drivers_route(current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if management_data['success']:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=management_data.get('error', 'Erro ao buscar entregadores'))
...
async def get_delivery_details_route(delivery_id: int, current_user: SystemUser = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if management_data['success']:
            ...
            all_deliveries = management_data['pending_deliveries'] + management_data['active_deliveries']

            delivery = next((d for d in all_deliveries if d['delivery_id'] == delivery_id), None)

            if delivery:
                return {'success': True, 'delivery': delivery}
            else:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Entrega #{delivery_id} não encontrada")
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=management_data.get('error', 'Erro ao buscar detalhes da entrega')
            )
...
async def delivery_health_check():
    ...
    """
    return {'status': 'healthy', 'service': 'delivery', 'timestamp': datetime.now().isoformat()}
